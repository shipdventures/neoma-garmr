...
### Added
- `WebhookSignatureGuard` for Svix-standard HMAC-SHA256 webhook signature verification
- `WebhookOptions` interface and optional `webhook` property on `GarmrOptions`

## [0.8.0] - 2026-04-05

### Added
- `GarmrModule.forRootAsync()` for resolving options via the NestJS DI container (e.g., `ConfigService`, typed config)
- Built on `ConfigurableModuleBuilder` — supports `useFactory`/`inject`/`imports`

### Changed
- `GarmrModule.forRoot()` now generated by `ConfigurableModuleBuilder` (identical public API, internal simplification)
...
[Unreleased]: https://github.com/shipdventures/neoma-garmr/compare/v0.8.0...HEAD
