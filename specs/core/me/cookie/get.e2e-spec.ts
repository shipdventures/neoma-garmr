...
import {
  MAGIC_LINK_AUDIENCE,
  SESSION_AUDIENCE,
} from "@lib/services/magic-link.service"
...
describe("GET /me (Cookie)", () => {
  ...
  describe("When both Bearer and Cookie are present", () => {
    it("should authenticate via Bearer (Bearer takes priority)", async () => {
      const bearerUser = await authenticateViaEmail(app, faker.internet.email())
      const cookieUser = await authenticateViaEmail(app, faker.internet.email())
